...
void setup(){
	...
	timerID = TimerSedes.setTimeout(30000, SedesFadeOut);				// 30s tyle będzie świecić po wykryciu ruchu
	//Reszta kodu inicjującego
...
*/
	...
	bme.setTempCal(0.7);// Temp was reading high so subtract 1 degree */
}
...
void loop(){
	...
	if (TimerSedes.isEnabled(timerID) && digitalRead(PIR_Sensor) == 1 && analogRead(PhotoResistor) < 450) {  //Returns true if the specified timer is enabled
		...
		SedesDimmer.on();
		timerID = TimerSedes.setTimeout(30000, SedesIlluminationOFF);      //Włączy illuminacje sedesu za 30s
		Serial.println("TimerSedes.setTimeout");
	}
		...
		Serial.print("TimerSedes.isEnabled(timerID) = ");
		Serial.print(TimerSedes.isEnabled(timerID));
...
void handleInterrupt() {
	PhotoResValue = analogRead(PhotoResistor);  //Odczyt natężenia światła
	Serial.println("RUCH wykryty");
	Serial.print("TimerSedes.isEnabled(timerID) = ");
	Serial.println(TimerSedes.isEnabled(timerID));
	Serial.print("PhotoResValue = ");
	Serial.println(PhotoResValue);
 
	if ( TimerSedes.isEnabled(timerID) && PhotoResValue != 500 ) {	//Returns true if the specified timer is enabled
		TimerSedes.restartTimer(timerID);	//Wydłuża illuminacje sedesu o kolejne 30s
		Serial.println("TimerSedes.restartTimer");
	}
	/*else if ( TimerSedes.isEnabled(timerID) && PhotoResValue > 500 ) {	//Returns true if the specified timer is enabled
		TimerSedes.disable(timerID);		//Wyłącza illuminacje sedesu bo wykrył że światło główne się świeci
		SedesDimmer.off();
	}*/
	else if ( !TimerSedes.isEnabled(timerID) && PhotoResValue < 450 ) {	//Returns true if the specified timer is enabled
		SedesDimmer.on();
		timerID = TimerSedes.setTimeout(4000, SedesIlluminationOFF);			//Włączy illuminacje sedesu za 30s
		Serial.println("TimerSedes.setTimeout");
	}
}

void SedesIlluminationON() {	//Powolne wygazsenie illuminacji sedesu, czas 0.5s
	SedesDimmer.on();
    Serial.println("Illuminacja włączona");
}
void SedesIlluminationOFF() {  //Powolne wygazsenie illuminacji sedesu, czas 0.5s
    SedesDimmer.off();
